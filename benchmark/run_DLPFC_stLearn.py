...
output_path = Path('/home/zzq/zzq/DeepST-main/Benchmark/stlearn')
...
for sample in sample_list:
    print("================ Start======================")
    OUTPUT_PATH = Path(f"/home/zzq/zzq/DeepST-main/Benchmark/result/{sample}")
...
# stSME
    ...
    st.pl.cluster_plot(data_, use_label="X_pca_kmeans",show_image=None)
    methods_ = "stSME_disk"
    ...
    df_meta['stLearn'] = data_.obs["X_pca_kmeans"].tolist()
    df_meta.to_csv(f'{OUTPUT_PATH}/metadata.tsv', sep='\t', index=False)
    df_meta = df_meta[~pd.isnull(df_meta['layer_guess'])]
    ARI = metrics.adjusted_rand_score(df_meta['layer_guess'], df_meta['stLearn'])
    print('===== Project: {} ARI score: {:.3f}'.format(sample, ARI))
    print("================ End ======================")
    ARI_list.append(ARI)
    ARI_list3.append(np.around(ARI, decimals=3))
print('===== Project: AVG ARI score: {:.3f}'.format(np.mean(ARI_list)))
print('===== Project: ARI score: ',ARI_list3)
